...
from algorithms.k_center_greedy import create_global_k_center_mask
...
DIRECT_SELECTION_ALGORITHMS = frozenset(
    {
...
class DataBundle:
    ...
    base_train_features: torch.Tensor | None = None
...
def prepare_data_bundle(
    *,
    ...
    num_workers: int,
    pretrained: bool,
    load_base_train_features: bool = False,
...
def validate_arguments(
    ...
    if (
        any(
            algorithm not in DIRECT_SELECTION_ALGORITHMS
        ...
        raise ValueError(
            ...
            "optimization algorithm is requested. SRS and KCG may use 0 "
            "because they perform no validation-fitness evaluations."
        )

    if (
        Algorithm.K_CENTER_GREEDY in args.algorithms
        and not args.use_features
    ):
        raise ValueError(
            "KCG requires '--use-features' because selection is performed in "
            "the frozen representation space."
        )
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Algorithms: SRS KCG RS LS GEN MEM, or their full enum names."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=100,
        help=(
            "Exact fitness budget for optimization algorithms. Use 0 when "
            "running only SRS and/or KCG because these direct baselines "
            "perform no validation-fitness evaluations. Default: 100."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Root directory for global k-center distance and traversal "
            "caches. Dataset/model/split subdirectories are added "
            "automatically. Default: FEATURES_DIR/k_center_cache."
        ),
    )

    parser.add_argument(
        "--k-center-distance-block-size",
        type=int,
        default=512,
        help=(
            "Number of pairwise-distance rows generated per GPU block when "
            "building the global k-center cache. Default: 512."
        ),
    )
...
def build_search_summary(
    *,
    ...
    return {
        ...
        "max_evaluations": (
            0
            if algorithm in DIRECT_SELECTION_ALGORITHMS
        ...
        "selection_strategy": (
            ...
            if algorithm == Algorithm.STRATIFIED_RANDOM
                ...
                if algorithm == Algorithm.K_CENTER_GREEDY
...
def execute_k_center_greedy_run(
    *,
    args: argparse.Namespace,
    experiment_name: str,
    run_id: str,
    ...
    """Generate and save one global k-center greedy mask."""
    start_time = time.perf_counter()

    try:
        if not bundle.uses_preextracted_features:
            raise ValueError(
                "KCG requires pre-extracted frozen feature vectors."
            )

        if bundle.base_train_features is None:
            raise ValueError(
                "The base-training feature matrix is unavailable."
            )
        ...
        if len(bundle.base_train_features) != bundle.total_instances:
            raise ValueError(
                "Base-training features are not aligned with the mask index "
                "space."
            )
        ...
        cache_root = (
            args.k_center_cache_dir
            if args.k_center_cache_dir is not None
            else features_root / "k_center_cache"
        )
        ...
        max_keep_percentage = max(
            args.keep_percentages
        )

        best_mask, diagnostics = create_global_k_center_mask(
            features=bundle.base_train_features,
            targets=bundle.base_train_targets,
            keep_percentage=keep_percentage,
            max_keep_percentage=max_keep_percentage,
            search_seed=search_seed,
            device=device,
            cache_dir=cache_dir,
            distance_block_size=
                args.k_center_distance_block_size,
        )
        ...
        available_by_class = diagnostics[
            "available_by_class"
        ]
        selected_by_class = diagnostics[
            "selected_by_class"
        ]
        ...
        class_distribution = {
            str(class_label): {
                "available_instances":
                    int(available_by_class[class_label]),
                "selected_instances":
                    int(selected_by_class[class_label]),
                "selected_fraction_within_class": (
                    selected_by_class[class_label]
                    / available_by_class[class_label]
                ),
            ...
            [
                {
                    ...
                    "Run ID": run_id,
                    "Algorithm":
                        Algorithm.K_CENTER_GREEDY.value,
                    "Dataset": dataset.value,
                    "Model": model_name.value,
                    "Target Metric": target_metric.value,
                    "Search Seed": search_seed,
                    ...
                    "Fitness": None,
                    "Uses Validation For Selection": False,
                    "Uses Class Labels For Selection": False,
                    "Generated Subsets": 1,
                    "Selection Scope": "global",
                    "Feature Normalization": "none",
                    "Distance Metric": "squared_euclidean",
                    "Initial Center Strategy":
                        "seeded_random_global_instance",
                    "First Center Position":
                        diagnostics["first_center_position"],
                    "Covering Radius":
                        diagnostics["covering_radius"],
                    "Distance Cache Reused":
                        diagnostics["distance_cache_reused"],
                    "Order Cache Reused":
                        diagnostics["order_cache_reused"],
                    "Selection Duration Seconds":
                        selection_duration,
                }
            ],
        ...
        write_json(
            run_dir / "best_fitness_history.json",
            {
                "best_fitness_history": [],
                "note": (
                    "Global KCG performs no validation-fitness evaluations."
                ),
            },
        )
        ...
        write_json(
            ...
            {
                "note": (
                    "Class labels were used only to report this distribution "
                    "after global k-center selection. They were not used by "
        ...
        summary.update(
            {
                ...
                "distance_cache_reused":
                    diagnostics["distance_cache_reused"],
                "distance_cache_build_seconds":
                    diagnostics["distance_cache_build_seconds"],
                "distance_cache_size_gib":
                    diagnostics["distance_cache_size_gib"],
                "order_cache_reused":
                    diagnostics["order_cache_reused"],
                "order_build_seconds":
                    diagnostics["order_build_seconds"],
                "max_cached_centers":
                    diagnostics["max_cached_centers"],
            }
        )
        ...
        logger.info(
            "Completed %s | global_k_center=true | "
            "fitness_evaluations=0 | selected_instances=%d | "
            "selection_seconds=%.3f | covering_radius=%.6f",
            run_id,
            selected_instances,
            selection_duration,
            diagnostics["covering_radius"],
...
def execute_search_run(
    *,
    ...
    if algorithm == Algorithm.K_CENTER_GREEDY:
        return execute_k_center_greedy_run(
            args=args,
            experiment_name=experiment_name,
            run_id=run_id,
            run_dir=run_dir,
        ...
        manifest = {
            ...
            "methodological_notes": {
                ...
                "global_k_center_greedy": {
                    ...
                    "retention_masks":
                        "nested_prefixes_of_one_order_per_seed",
                    "distance_cache_dtype": "float32",
                    "distance_block_size":
                        args.k_center_distance_block_size,
                },
                "data_representation": (
